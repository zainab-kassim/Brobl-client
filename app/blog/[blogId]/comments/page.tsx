...
export default function comments({ params }: { params: { blogId: string } }) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    try {
      ...
      if (username && token) {
        ...
        const res = await axios.post(`http://localhost:4000/api/blog/${params.blogId}/comment/create`, {
          text: values?.text
  ...
  async function handleDeleteComment(commentId: string) {
    try {
      ...
      const headers = createAuthHeaders(token)
      if (username && token) {
        const response = await axios.delete(`http://localhost:4000/api/blog/${params.blogId}/comment/${commentId}/delete`,
          { headers })
  ...
  async function handleDeleteBlog() {
    try {
      ...
      const headers = createAuthHeaders(token)
      if (username && token) {
        const response = await axios.delete(`http://localhost:4000/api/blog/${params.blogId}/delete`,
          { headers })
        const { message } = response.data
        toast({
          description: message
